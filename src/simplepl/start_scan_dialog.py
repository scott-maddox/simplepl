...
class StartScanDialog(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        min = float(self._settings.value('wavelength/min', 800.))
        max = float(self._settings.value('wavelength/max', 5500.))
        precision = float(self._settings.value(
                                             'wavelength/precision', 0.1))
        decimals = floor(-log10(precision))
        if decimals < 0:
            decimals = 0
        start = float(self._settings.value('scan/start', min))
        stop = float(self._settings.value('scan/stop', max))
        ...
        self.startSpinBox.setRange(min, max)
        ...
        self.stopSpinBox.setRange(min, max)
